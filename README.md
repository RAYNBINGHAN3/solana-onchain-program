...
`second-anchor` 是一个基于 Solana-Anchor 框架实现的多 DEX 智能套利（Onchain Arbitrage）合约。它能够扫描并利用主流 DEX 池模式（如 CLMM, CPMM, DAMM, DLMM, PUMP, RAYDIUM, WHIRLPOOL 等）进行套利。

### 核心功能
- **多池模式支持**: CLMM、DLMM、CPMM、DAMMV2、PUMP、Raydium、Whirlpool。
- **全局路径评估**: 扫描多代币、多池子的行情，评估全局套利机会。
...
`second-anchor` is a multi-DEX intelligent arbitrage (Onchain Arbitrage) contract implemented with the Solana-Anchor framework. It supports price scanning and arbitrage across major DEX pool models (e.g., CLMM, CPMM, DAMM, DLMM, PUMP, RAYDIUM, WHIRLPOOL).
