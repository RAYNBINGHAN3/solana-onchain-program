...
const provider = anchor.AnchorProvider.local("https://rpc.shyft.to?api_key=fE59G3v0thhXEpgT", {
  preflightCommitment: "processed",
});
anchor.setProvider(provider);

// 2. 手动指定 programId （因为直接跑不会自动加载）
const programId = new anchor.web3.PublicKey("2dDzSGtvn2d46asoSU733SL6hRRNS2waMrC2rDksSs4s");

// 3. new Program（不依赖 anchor.workspace）
const program = new anchor.Program<ZooeyGo>(idl as ZooeyGo, provider);
...
it("Is initialized!", async () => {
  ...
  if (!tokenMint3TokenAccountInfo) {
    console.log(`🔧 创建tokenMint3TokenAccount: ${tokenMint3TokenAccount.toString()}`);
    //创建tokenMint3TokenAccount
    const createTokenMint3TokenAccountTx = await createAssociatedTokenAccountIdempotentInstruction(
      provider.publicKey,
      tokenMint3TokenAccount,
      provider.publicKey,
      tokenMint3,
  ...
  // DLMM池 - 自动解析所有账户
  const dlmmPool2_4 = new PublicKey('6wJ7W3oHj7ex6MVFp2o26NSof3aey7U8Brs8E371WCXA');
