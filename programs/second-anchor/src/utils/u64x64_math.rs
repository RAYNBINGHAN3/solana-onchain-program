...
pub fn get_fee_in_period(
    ...
    let bps = u128::from(reduction_factor)
        ...
        .unwrap();
    let base = ONE.checked_sub(bps).unwrap();
    let result = pow(base, passed_period.into()).ok_or_else(|| ErrorCode::MathOverflow)?;

    let (fee, _) = result
        .checked_mul(cliff_fee_numerator.into())
        .unwrap()
        .overflowing_shr(SCALE_OFFSET as u32);

    let fee_numerator = u64::try_from(fee).map_err(|_| ErrorCode::MathOverflow)?;
    Ok(fee_numerator)
}
