...
use crate::utils::u128x128_math::{mul_div_u256, safe_mul_div_cast, Rounding};
use crate::utils::u64x64_math::{get_fee_in_period, ONE};
...
pub fn parse_dammv2_pool_data(
    ...
    // padding_0: [u8; 5] = 9..14
    let number_of_period = u16::from_le_bytes(pool_data[22..24].try_into().unwrap());
    let period_frequency = u64::from_le_bytes(pool_data[24..32].try_into().unwrap());
    let reduction_factor = u64::from_le_bytes(pool_data[32..40].try_into().unwrap());
    // padding_1: u64 = 32..40
    ...
    // );
    let variable_fee_control = u32::from_le_bytes(pool_data[68..72].try_into().unwrap());
    let bin_step = u16::from_le_bytes(pool_data[72..74].try_into().unwrap());
    // 其他DynamicFeeStruct字段...
    let volatility_accumulator = u128::from_le_bytes(pool_data[120..136].try_into().unwrap());
    // padding_1: [u64; 2] = 96..112

    // 解析Pool主体字段 (偏移量168开始: 8 + 160 = 168)
    // let pool_offset = 168;

    let token_a_mint = &pool_data[168..200];
    let token_b_mint = &pool_data[200..232];

    let has_wsol_pool = token_a_mint == WSOL_MINT || token_b_mint == WSOL_MINT;
    pool_state.has_wsol_pool = has_wsol_pool;

    pool_state.token_a_mint = token_a_mint.try_into().unwrap();
    pool_state.token_b_mint = token_b_mint.try_into().unwrap();
    ...
    let activation_point = u64::from_le_bytes(pool_data[472..480].try_into().unwrap());
...
pub fn calculate_dammv2_price(pool: &mut Dammv2PoolState) -> Result<()> {
...
pub fn calculate_dammv2_total_fee_rate(
    ...
    // 3. 计算总费率 (基础费率 + 动态费率)
    let total_fee_numerator = base_fee_numerator.checked_add(variable_fee as u64).unwrap();
...
/// DAMM-V2 Quote函数 - Token输入，WSOL输出 (完全复刻官方SDK)
pub fn dammv2_quote_exact_input_token1_output_token2(
    pool: &Dammv2PoolState,
    a_to_b: bool,
    token_amount_in: u64,
    current_point: u64,
    ...
    //check token 2022 fee
    let transfer_fee = get_transfer_fee(token2_mint_info, final_output)?;
    final_output = match transfer_fee {
        0 => final_output,
        _ => final_output.checked_sub(transfer_fee).unwrap(),
    };
...
/// A->B交换计算
fn get_swap_result_from_a_to_b(sqrt_price: u128, liquidity: u128, amount_in: u64) -> Result<u64> {
...
/// B->A交换计算
fn get_swap_result_from_b_to_a(sqrt_price: u128, liquidity: u128, amount_in: u64) -> Result<u64> {
    // 计算新的目标价格: √P' = √P + Δy / L
